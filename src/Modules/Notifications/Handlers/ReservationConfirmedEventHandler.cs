...
internal sealed class ReservationConfirmedEventHandler(
    IReservationsModuleApi reservationsModuleApi,
    IUsersModuleApi usersModuleApi,
    ITemplateService templateService,
    IEmailSender emailSender,
    ILogger<ReservationConfirmedEventHandler> logger) : INotificationHandler<ReservationConfirmedEvent>
{
    public async Task Handle(ReservationConfirmedEvent notification, CancellationToken cancellationToken)
    {
        logger.LogInformation("Domain Event: Reservation confirmed with Id {ReservationId}", notification.ReservationId);
        ...
        var emailBody = await templateService.RenderAsync("ReservationConfirmed", placeholders, cancellationToken);
